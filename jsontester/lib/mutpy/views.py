...
class TextView(QuietTextView):
    ...
    def mutation(self, number, mutations, module, mutant):
        for mutation in mutations:
            self.level_print(
                '[#{:>4}] {:<3} {}:{:<3}: '.format(number, mutation.operator.name(), module[0], mutation.node.lineno),
                ended=False,
                level=2,
            )
            ...
            if self.show_mutants:
                self.print_code(mutant, number, mutation.node.lineno, module[1], module[0])

    def mutation1(self, number, mutations, module, mutant):
        for mutation in mutations:
            self.level_print(
